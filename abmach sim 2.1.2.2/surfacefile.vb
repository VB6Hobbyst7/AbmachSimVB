    ...
    Public Class surfacefile
        ...
        Public Sub filesave(ByVal fileHeader As String, ByVal FileName As String, ByVal surf(,) As Single, _
            ...
            Try
                ...
                '    Next j
                'Next i
                m_streamWriter.WriteLine("y midpoint cross section")
                j = (surf.GetUpperBound(1) - 1) / 2
                For i = 1 To surf.GetUpperBound(0) - 1 Step meshstep
                    m_streamWriter.WriteLine(Math.Round(getxlocation(i, meshsize, ccomp, pathmin_x), 5) & "," & Math.Round(getylocation(j, meshsize, ccomp, pathmin_y), 5) & "," & Math.Round(surf(i, j), 5))

                Next i
                m_streamWriter.WriteLine("x midpoint cross section")
                i = (surf.GetUpperBound(0) - 1) / 2
                For j = 1 To surf.GetUpperBound(1) - 1 Step meshstep
                    m_streamWriter.WriteLine(Math.Round(getxlocation(i, meshsize, ccomp, pathmin_x), 5) & "," & Math.Round(getylocation(j, meshsize, ccomp, pathmin_y), 5) & "," & Math.Round(surf(i, j), 5))
        ...
        Function readcsvsurffile(ByVal input As Form1.fileType, ByVal surfinptfilename As String) As Boolean
            ...
            Dim fileok, headersloaded As Boolean

            Dim deltay, deltax, mindeltax, mindeltay As Double
            ...
            Try
                ...
                While Not (strLine Is Nothing)
                    ...
                    ElseIf filetype = 3 And headersloaded Then
                        ...
                        End If
                    ElseIf filetype = 4 And headersloaded Then
                        If IsNumeric(strLine.Substring(0, 2)) Then
                            Dim words() As String
                            words = strLine.Split(",")
                            csvPoint(a).x = CDbl(words(0))
                            csvPoint(a).y = CDbl(words(1))
                            csvPoint(a).z = CDbl(words(2))
                            csvPoint(a).depth = CDbl(words(3))
                            csvPoint(a).mi = 1
                            a += 1
                ...
                Next
                ReDim targetCurve(csvPoint.Length)
                Dim actual As Integer
                actual = 0
                For i As Integer = 0 To csvPoint.Length - 1
                    If Not (csvPoint(i).depth = 0) Then
                        targetCurve(actual).X = csvPoint(i).x
                        targetCurve(actual).Fx = csvPoint(i).depth
                        actual += 1
                    End If
                Next
                ReDim Preserve targetCurve(actual - 1)
                ...
                With csvExtents
                    ...
                    For a = 0 To .size - 1
                        ...
                        Else
                            tsurface(column, row) = csvPoint(a).depth
                        End If
        ...
        Function readDxfSurfFile(ByVal input As Form1.fileType, ByVal dxfinptfilename As String) As Boolean
            pointcount = 0
            Dim a, b, mask As Integer
            Dim strLine, strline1 As String
