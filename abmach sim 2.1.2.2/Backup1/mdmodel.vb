...
Namespace abmach
    Module mdmodel
        ...
        Dim gaussWidth, incidentAngle, cutoffAngle As Double
        Dim g1, gb As Double
                ...
                For b = 0 To jetradius * 2
                    ...
                    If mrrtype = 6 Then
                        If checklocation <= 1.0 Then
                            mrrvalue(a, b) = CDbl(factor * gaussian(1, gb, checklocation))
                        Else
                            mrrvalue(a, b) = 0
                        End If
                    Else
                        If checklocation <= 1.0 Then
                            mrrvalue(a, b) = CDbl(factor * mrr(checklocation))

                        Else
                            mrrvalue(a, b) = 0
                        End If
                    End If
                    'DEBUG.writeline(a & "," & b & "," & mrrvalue(a, b))
                Next
            Next
        End Sub 'initialize
        ...
        Public Sub calc_arrray_size()
            ...
            'DEBUG.writeline("sizex:" & size_xarray & " ,sizey:" & size_yarray)
            'DEBUG.writeline(pathmin_x & " |" & pathmax_x & " |" & pathmin_y & " |" & pathmax_y)
            'DEBUG.writeline("")
        End Sub 'calc_arrray_size
        Sub fillarrays(ByVal length As Integer)

            'fill toolpath array with x,y and f,b,c,z
            Dim i, j, segment As Integer
            segment = 0
            For i = 1 To length
                'Debug.WriteLine("i" & i & "| segment" & segment)

                If nc(i).gcode = 1 Then straight(i, segment)
                ' If nc(i).bvalue - nc(i - 1).bvalue <> 0 Or nc(i).cvalue - nc(i - 1).cvalue <> 0 Then axis5nc(i, segment)
        ...
        Public Sub arc(ByVal a As Integer, ByRef b As Integer)
            'arcs in nc code
            Dim deltax, deltay, deltaz, rad, cdist, theta1, xtemp, theta_t, arc_l As Double
        ...
        Public Sub axis5nc(ByVal a As Integer, ByRef b As Integer)
            'straight lines in nc code
            Dim deltax, deltay, deltaz, deltac, deltab, dist, dist_cur As Double
            Dim num_seg, d As Integer
            deltax = nc(a).x - nc(a - 1).x
            deltay = nc(a).y - nc(a - 1).y
            ...
            End If
        End Sub 'axis5nc
        Function direction(ByVal x1 As Double, ByVal y1 As Double, ByVal x2 As Double, _
        ...
        Sub accelcheck(ByVal maxacceln As Double, ByVal maxaccelt As Double)
            ...
            Dim i, j, total_segments As Integer
            For i = 1 To total_segments - 2
            ...
            For i = total_segments - 2 To 2 Step -1
                deltax = toolpath(i).x - toolpath(i - 1).x
                deltay = toolpath(i).y - toolpath(i - 1).y
        ...
        Public Sub mrrxmlselect(ByVal type As Integer)
            Dim sdir As String = System.IO.Path.GetDirectoryName(Application.ExecutablePath)
            Dim spath As String = sdir & System.IO.Path.DirectorySeparatorChar & mrrcoeffsxmlfile
            Dim teststring As String
            Dim testarray(8) As Double

            Try
                Dim xmlreader As New XmlTextReader(spath)
        ...
        Sub arrayStats(ByVal arr(,) As Double, ByRef localAve As Double, ByRef localStdDev As Double, _
        ...
        Sub arrayStats(ByVal arr(,) As Single, ByRef localAve As Double, ByRef localStdDev As Double, _
                       ByRef min As Double, ByRef max As Double)
        ...
        Function maxdepth(ByVal x As Double, ByVal y As Double, ByVal direction As groovedirection) As Double
            ...
            Dim i, j, aa, xIndex, yIndex, ba As Integer
            Dim temp As Double
            ...
            Try
                ...
                If direction = groovedirection.X Then
                    ...
                    temp = 0
                    For ba = wy1 To wy2
                        'If Math.Abs(surface(xIndex, ba)) > Math.Abs(temp) Then temp = surface(xIndex, ba)
                ...
                If direction = groovedirection.Y Then
                    ...
                    temp = 0
                    For aa = wx1 To wx2
                        'If Math.Abs(surface(aa, yIndex)) > Math.Abs(temp) Then temp = surface(aa, yIndex)
        ...
        Sub loadxmlmachineinfo()
            Dim sdir As String = System.IO.Path.GetDirectoryName(Application.ExecutablePath)
            Dim sheaderpath As String = sdir & System.IO.Path.DirectorySeparatorChar & smachinetagsxmlfile
            OpenXML(sheaderpath)
        ...
        End Sub 'xydistance
        Function distance(ByVal x1 As Double, ByVal y1 As Double, ByVal z1 As Double, _
        ...
        Function getyindex(ByVal yi As Double, ByVal meshsize As Double, ByVal pad As Double, _
        ...
        Function getylocation(ByVal yi As Long, ByVal meshsize As Double, ByVal pad As Double, _
                                ByRef miny As Double) As Double
            'convert Y array index to location
            getylocation = ((yi * meshsize) - pad) + miny
        End Function

        Function getxindex(ByVal xi As Double, ByVal meshsize As Double, ByVal pad As Double, _
        ...
        Function subtract_surface(ByVal r As Integer) As Boolean
            Dim a, b, i, j, yindex, xindex, yindex3, xindex3 As Integer
            ...
            Try
                ...
                For i = 1 To total_segments - 1
                    ...
                    If deltab <> 0 Then
                        ...
                        deltal3d = Math.Sqrt(deltax * deltax + deltay * deltay + deltaz * deltaz)
                        deltax = toolpath(i).x - toolpath(i - 1).x
                        deltay = toolpath(i).y - toolpath(i - 1).y
                    ...
                    xindex3 = getxindex(toolpath(i - 1).x, mesh_size, ccomp, pathmin_x)
                    yindex3 = getyindex(toolpath(i - 1).y, mesh_size, ccomp, pathmin_y)
                    delta_index = Math.Sqrt((xindex - xindex3) * (xindex - xindex3) + (yindex - yindex3) * (yindex - yindex3)) / 2
                    ...
                    If delta_index <> 0 Then
                        ...
                        For a = xindex - jetradius To xindex + jetradius
                            For b = yindex - jetradius To yindex + jetradius
                                ...
                                If mrrvalue(a - xindex + jetradius, b - yindex + jetradius) <> 0 Then
                                    '6/22/2007 added mi to calc for variable mi
                                    If maskSurface(xindex, yindex) <> 0 Then
                                        tempsurface(a, b) = fr * slopefactor3(a, b) * mi_surface(a, b) * mrrvalue(a - xindex + jetradius, b - yindex + jetradius)
                                    Else
                                        tempsurface(a, b) = 0
                                    End If
                                Else
                        ...
                        For a = xindex - jetradius To xindex + jetradius
                            For b = yindex - jetradius To yindex + jetradius
                                If (surface(a, b) < surface(a + 1, b) And surface(a, b) < surface(a - 1, b) And _
                                    surface(a, b) < surface(a, b + 1) And surface(a, b) < surface(a, b - 1)) Or _
                                    (surface(a, b) > surface(a + 1, b) And surface(a, b) > surface(a - 1, b) And _
        ...
        Function newfeedout(ByVal targsurface As surfacetype, ByVal newfeed As Boolean) As Boolean
            Dim nomdepthtable As Boolean
            'calc new feed rates for each nc line based on depth and nominal depth
            Dim i, j, a, masktemp, imax, clipFlagCount As Integer
            'If strgroovedir = "MULTI" Then

            Try
                imax = nc.GetUpperBound(0)
                j = 0
                For i = 1 To imax
                    a = 0
                    nc(i).depth = 0
                    nc(i).minsegmentdepth = 0
                    minsegmentdepthj = 999999
                    nc(i).maxsegmentdepth = 0
                    maxsegmentdepthj = 0
                    ...
                    nc(i).mask = 0
                    masktemp = 0
                    Do While toolpath(j).n = nc(i).n
                        masktemp = maskSurface(getxindex(toolpath(j).x, mesh_size, ccomp, pathmin_x), getyindex(toolpath(j).y, mesh_size, ccomp, pathmin_y))
                        If masktemp <> 0 Then
                            nc(i).mask = 1
                        ...
                        If j > toolpath.GetUpperBound(0) Then
                            Debug.WriteLine("j:" & j)
                            Exit Do
                        End If
                    ...
                    If newfeed Then
                        ...
                        If nc(i).depth = 0 And nc(i).mask <> 0 Then
                            Throw New Exception("Cannot Calculate New Feedrates Model depth = 0")
                        End If

                        If (nc(i).nominalsurface = 0 Or nom_depth = 0) And nc(i).mask <> 0 Then
                            Throw New Exception("Target Depth =0")
                        End If

                        If nc(i).mask <> 0 Then
                            nc(i).newf = nc(i).f * Math.Abs(nc(i).depth / nc(i).nominalsurface)
                        Else
                            nc(i).newf = nc(i).f
                        End If
                        ...
                        If nc(i).newf = 0 Then
                            Throw New Exception("New Feed rate = 0")
                        End If
                ...
                Next i
                If clipfrflag Then
                    MessageBox.Show("Machine Maximum Feedrate Exceeded. Changed to " & machine.maxspeed & " in " & clipFlagCount & " lines.", "Warning")
                End If
            Catch ex As Exception
        ...
        Private Function slopefactor3(ByVal a As Integer, ByVal b As Integer) As Double

            's1z = (surface(a + 1, b) - surface(a - 1, b)) * (surface(a + 1, b) - surface(a - 1, b)) * meshsquared
            's2z = (surface(a, b + 1) - surface(a, b - 1)) * (surface(a, b + 1) - surface(a, b - 1)) * meshsquared
            's1.z = surface(a + 1, b) - surface(a - 1, b)
            's2.z = surface(a, b + 1) - surface(a, b - 1)
            'normal = cross(s1, s2)

            'gaussWidth = 0.6
            'slopefactor is zero at greater than than cutoffAngle

            'cutoffAngle = pi * 70 / 180
            'calc angle between jet vector and surface normal
            'incident angle is 0 when jet is normal to surface 
            'incidentAngle = Math.Abs(Math.Acos(normal.z / mag(normal)))
            'If incidentAngle > cutoffAngle Then
            'slopefactor3 = 0
            ' Else
            'slopefactor3 = Math.Exp(-1 * gaussWidth * (incidentAngle - crit_angle_1_rad) ^ 2)
            'slopefactor3 = Math.Sqrt(Math.Abs(Math.Cos(incidentAngle - crit_angle_1_rad)))
            slopefactor3 = Math.Abs(Math.Cos(Math.Acos(meshsquared / Math.Sqrt((surface(a + 1, b) - surface(a - 1, b)) _
                         * (surface(a + 1, b) - surface(a - 1, b)) * meshsquared + (surface(a, b + 1) _
                         - surface(a, b - 1)) * (surface(a, b + 1) - surface(a, b - 1)) * _
                         meshsquared + meshsquared * meshsquared)) - crit_angle_1_rad))
            'End If

            'Debug.WriteLine(incidentAngle.ToString & "|" & slopefactor3.ToString)
